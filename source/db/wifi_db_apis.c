        ...
        struct schema_Wifi_VAP_Config *new_rec)
{
	...
	vap_array_index = convert_vap_index_to_vap_array_index(&g_wifidb->hal_cap.wifi_prop, vap_index);
	g_wifidb->radio_config[radio_index].vaps.rdk_vap_array[vap_array_index].config_status = webconfig_apply_status_success;
	wifi_util_dbg_print(WIFI_CTRL, "%s:%d: Setting the status as success for vap %d in vap_array_index %d\n", __func__, __LINE__, vap_index, vap_array_index);
    }
    else
    {
        wifi_util_dbg_print(WIFI_DB,"%s:%d:Unknown\n", __func__, __LINE__);
