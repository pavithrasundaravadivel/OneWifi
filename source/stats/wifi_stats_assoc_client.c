    ...
    if (hal_sta != NULL) {
        for (i = 0; i < num_devs; i++) {
            ...
            if (link_data && i < num_devs && ((link_quality_measurement) || (rf_down_mesh_sta))) {
                ...
                /* Raw cumulative EAPOL M1-M4 counts only - OneWifi computes no attempts
                 * and no failures. WEI counts eap_attempts from the per-M1
                 * wifi_event_hal_eap_status_code event and eap_failures from the
                 * disconnect reason. */
