...
/* LQ/SC pillar is active when either RFC flag is set, or RF link is down (mesh backhaul), or EM node. */
static bool lq_sc_event_allowed(void)
{
    wifi_ctrl_t *ctrl = get_wifictrl_obj();
    wifi_rfc_dml_parameters_t *rfc = get_ctrl_rfc_parameters();
    if (ctrl == NULL || rfc == NULL) return false;
    return (rfc->wei_rfc_mask & (WEI_RFC_SC | WEI_RFC_LQ)) ||
           ctrl->rf_status_down ||
...
int exec_event_link_quality(wifi_app_t *apps, wifi_event_subtype_t sub_type, void *arg, int len)
{
    switch (sub_type) {
        ...
        case wifi_event_exec_timeout:
            if (!lq_sc_event_allowed()) {
                wifi_util_dbg_print(WIFI_APPS, "%s:%d LQ/SC RFC disabled, dropping exec_timeout\n",
                    __func__, __LINE__);
                break;
            }
...
int exec_event_hal_ind(wifi_app_t *apps, wifi_event_subtype_t sub_type, void *arg)
{
    ...
    switch (sub_type) {
        ...
        case wifi_event_exec_stop:
            if (!lq_sc_event_allowed()) {
                wifi_util_dbg_print(WIFI_APPS, "%s:%d LQ/SC RFC disabled, dropping exec_stop\n",
                    __func__, __LINE__);
                break;
            }
            wifi_util_info_print(WIFI_APPS," %s:%d\n",__func__,__LINE__);
